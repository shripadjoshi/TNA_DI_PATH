...
function writetxthoover(text){	
if(isloaded){
if(text!=0){
text="<div class='navtext2'>"+text+" </div><div class='margine'></div>";
ishover=true;
if(NS4)text='<div class="navtextall">'+((centertext)?'<center>':'')+text+((centertext)?'</center>':'')+'</div>';
writeindiv(text);
getboxheight();
if((W3C || IE4) && dofade){
ieop=0;
...
op_id=setTimeout('incropacity()', 0);
}}
...
xoff=(center)? mx-boxwidth/2 : mx+5 - 210;
yoff=(my+boxheight+30-getpagescrolly()+margin>=w_y)? -15-boxheight: 30-40;
